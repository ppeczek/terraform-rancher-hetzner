resource "null_resource" "rancher" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker run -d --name rancher-server --restart=unless-stopped --volumes-from rancher-data -p 0.0.0.0:80:80 -p 0.0.0.0:443:443 rancher/rancher:v2.4.3 --acme-domain ${format(var.hostname_format, 1)}.${var.domain}",
    ]
  }
}